...
#     def update_player(self):
#         self.player = self.controller.get_attribute('player')
#         print(f"Player updated to: {self.player}")
#
#     def update_opponent(self):
#         self.opponent = self.controller.get_attribute('opponent')
#         print(f"Opponent updated to: {self.opponent}")
#
#     def update_draw(self):
#         self.draw = self.controller.get_attribute('draw')
#         print(f"Draw updated to: {self.draw}")
#
#     def update_winner(self):
#         self.winner = self.controller.get_attribute('winner')
#         print(f"Winner updated to: {self.winner}")
#
#     def update_win_streak(self):
#         self.win_streak = self.controller.get_attribute('win_streak')
#         print(f"Win streak updated to: {self.win_streak}")
#
#     def update_turn(self):
#         self.turn = self.controller.get_attribute('turn')
#         print(f"Turn updated to: {self.turn}")
#
#     def update_allowed_moves(self):
#         self.allowed_moves = self.controller.get_attribute('allowed_moves')
#         print(f"Allowed moves updated to: {self.allowed_moves}")
#
#     def update_played_moves(self):
#         self.played_moves = self.controller.get_attribute('played_moves')
#         print(f"Played moves updated to: {self.played_moves}")
#
#     def update_player_move(self):
#         self.player_move = self.controller.get_attribute('player_move')
#         print(f"Player move updated to: {self.player_move}")
#
#     def update_move_sent(self):
#         self.move_sent = self.controller.get_attribute('move_sent')
#         print(f"Move sent updated to: {self.move_sent}")
#
#     def update_opponent_move(self):
#         self.opponent_move = self.controller.get_attribute('opponent_move')
#         print(f"Opponent move updated to: {self.opponent_move}")
#
#     def launch_ui(self):
#         QTimer.singleShot(0, self._show_ui)
#
...
#                     print(f"Server or opponent has dropped the game. End of game session.")
#                     self.cleanup_and_exit()
#                 except (socket.error, json.JSONDecodeError) as e:
#                     print(f"Disconnected from the server due to error: {e}.")
#                     self.cleanup_and_exit()
#
#         except Exception as e:
...
#             self.controller.set_attribute('player', self.player)
#             print(f"controller player set to = {self.controller.get_attribute('player')}")
#             self.opponent = 'O' if self.player == 'X' else 'X'
#             print(f"your opponent is player {self.opponent}")
#             self.controller.set_attribute('opponent', self.opponent)
#             print(f"controller opponent set to = {self.controller.get_attribute('opponent')}")
#
#         if message["subject"] == ALLOWED:
#             self.allowed_moves = message["data"]
#             print(f"the allowed moves = {self.allowed_moves}")
#             self.controller.set_attribute('allowed_moves', self.allowed_moves)
#             print(f"controller allowed moves set to = {self.controller.get_attribute('allowed_moves')}")
#             for n in ALL_MOVES:
#                 if n not in self.allowed_moves and n not in self.played_moves:
#                     self.played_moves.append(n)
#             self.controller.set_attribute('played_moves', self.played_moves)
#
#         if message["subject"] == GAME_OVER:
#             if message["data"] == "d":
#                 self.controller.set_attribute('draw', True)
...
#                 self.turn = True
#                 self.controller.set_attribute('turn', self.turn)
#                 print(f"is it your turn: {self.turn}")
#                 print(f"controller turn set to = {self.controller.get_attribute('turn')}")
#                 self.your_move()
#             else:
#                 self.turn = False
#                 self.controller.set_attribute('turn', self.turn)
...
#             if self.controller.get_attribute('move_sent'):
#                 self.player_move = self.controller.get_attribute('player_move')
#                 self.client.send(self.player_move.encode('utf-8'))
#                 self.turn = False
#                 self.controller.set_attribute('turn', self.turn)
#                 timer.stop()
#
#         timer = QTimer()
#         timer.timeout.connect(check_move)
...
#     controller = Controller()
#     client = TicTacToeClient(controller, HOST, PORT)
#     listener_thread = threading.Thread(target=client.listen_to_server, daemon=True)
